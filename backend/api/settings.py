...
if DATABASE_URL:
    DATABASES = {
        'default': dj_database_url.parse(
            DATABASE_URL,
            conn_max_age=600,
            # SSL is controlled via ?sslmode= in DATABASE_URL itself,
            # not hardcoded here — allows Docker Postgres (no SSL) and
            # managed cloud DBs (SSL required) to both work correctly.
        )
    }
else:
    DATABASES = {
