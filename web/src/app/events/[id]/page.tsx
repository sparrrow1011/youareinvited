...
import EventSharingSection from '@/components/EventSharingSection';
        ...
        <section className="px-4 sm:px-6 lg:px-12 py-8 lg:py-10 max-w-7xl mx-auto pb-24">
          ...
          {activeTab === 'guests' && (
            <EventGuestsSection
              ...
              bulkSendLoading={bulkSendLoading}
              bulkDeleteLoading={bulkDeleteLoading}
              onSearchChange={setInvitationSearch}
              onRsvpFilterChange={(value) => { setRsvpFilter(value); setInvitationPage(1); }}
              onCheckInStatusFilterChange={(value) => { setCheckInStatusFilter(value); setInvitationPage(1); }}
              onOpenStatusFilterChange={(value) => { setOpenStatusFilter(value); setInvitationPage(1); }}
              onPageChange={setInvitationPage}
              onSelect={handleSelectInvitation}
              onSelectAll={handleSelectAll}
              onAddGuest={openAddForm}
          ...
          {activeTab === 'design' && (
            <EventDesignSection
              hasBackgroundImage={Boolean(event?.background_image)}
              backgroundImageUrl={event?.background_image ? resolveMediaUrl(event.background_image) : null}
              templateSuccess={templateSuccess}
              onTemplateFileChange={handleTemplateFileChange}
              isPro={user?.plan === 'pro'}
              selectedTheme={selectedTheme}
              themeData={themeData}
              onThemeSave={handleSaveTheme}
              onThemeImageUpload={handleUploadThemeImage}
              onThemeImageClear={handleClearThemeImage}
              savingTheme={savingTheme}
              themeSaved={themeSaved}
              themeImageSavingField={savingThemeImageField}
              eventName={event?.name}
              eventDate={event?.date}
              themeHeroImage={event?.theme_hero_image ? resolveMediaUrl(event.theme_hero_image) : null}
              themeSecondaryImage={event?.theme_secondary_image ? resolveMediaUrl(event.theme_secondary_image) : null}
            />
          )}
          ...
          {activeTab === 'photos' && event && (
            <EventPhotosSection
              ...
              error={photosError}
              showVenueQr={showVenueQr}
              venueQrBlobUrl={venueQrBlobUrl}
              onToggleVenueQr={setShowVenueQr}
              ...
              downloadAllUrl={eventService.photosDownloadUrl(event.id)}
            />
          )}

          {activeTab === 'guest_app' && (
            <EventGuestAppSection
              form={guestAppForm}
              onChange={setGuestAppForm}
              saving={savingGuestApp}
              saved={guestAppSaved}
              onSave={handleSaveGuestApp}
            />
          )}
          ...
          {activeTab === 'sharing' && (
            <EventSharingSection
              event={event}
              stats={stats}
              securityPinSet={securityPinSet}
              securityPin={securityPin}
              savingPin={savingPin}
              pinSaveStatus={pinSaveStatus}
              onSecurityPinChange={(value) => { setSecurityPin(value); setPinSaveStatus('idle'); }}
              onSavePin={handleSavePin}
              onClearPin={handleClearPin}
              onCopyStaffLink={handleCopyStaffLink}
              waTemplate={waTemplate}
              savingWaTemplate={savingWaTemplate}
              waTemplateSaved={waTemplateSaved}
              waTemplateError={waTemplateError}
              onWaTemplateChange={(value) => { setWaTemplate(value); setWaTemplateSaved(false); }}
